...
class RateLimitedQueue {
  constructor({
    ...
    this.batchSizeMin = Math.max(1, toFiniteNumber(process.env.WA_BATCH_SIZE_MIN, 10));
    this.batchSizeMax = Math.max(this.batchSizeMin, toFiniteNumber(process.env.WA_BATCH_SIZE_MAX, 15));
    this.batchPauseMinMs = Math.max(0, toFiniteNumber(process.env.WA_BATCH_PAUSE_MIN_MS, 30000));
    this.batchPauseMaxMs = Math.max(this.batchPauseMinMs, toFiniteNumber(process.env.WA_BATCH_PAUSE_MAX_MS, 60000));
    this.maxRetries = Math.max(0, toFiniteNumber(maxRetries, 0));
    this.retryDelayMs = Math.max(0, toFiniteNumber(retryDelayMs, 30000));
  ...
  _save() {
    ...
    try {
      ...
      const items = this._queue.map(q => ({ data: q.data, meta: q.meta, attempts: q.attempts || 0 }));
  ...
  async _run() {
    while (this._queue.length > 0) {
      ...
      try {
        ...
        if (e?.retryable && (item.attempts || 0) < this.maxRetries) {
          item.attempts = (item.attempts || 0) + 1;
          this.logger.warn?.(`[${this.name}] retryable error, retry ${item.attempts}/${this.maxRetries} in ${this.retryDelayMs}ms:`, e?.message || e);
          if (this.storageFile && !item.fn) this._save();
