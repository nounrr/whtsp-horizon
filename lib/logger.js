...
function getSentMessages(options = {}) {
  try {
    ...
    // Lire reminders.json (anciens logs)
    if (fs.existsSync(REMINDER_LOGS_FILE)) {
      try {
        const content = fs.readFileSync(REMINDER_LOGS_FILE, 'utf8');
        let logs = JSON.parse(content);
    ...
    // Lire eelogs.log (nouveaux logs)
    const EEL_LOGS_FILE = path.join(LOG_DIR, 'eelogs.log');
    if (fs.existsSync(EEL_LOGS_FILE)) {
      try {
        const lines = fs.readFileSync(EEL_LOGS_FILE, 'utf8').split('\n').filter(Boolean);
        const eelogs = lines.map(line => {
          try { return JSON.parse(line); } catch { return null; }
        }).filter(e => e && e.status === 'success');
        let filtered = eelogs;
        if (options.date) {
          filtered = eelogs.filter(log => {
            const logDate = DateTime.fromISO(log.timestamp).toISODate();
            return logDate === options.date;
          });
        }
        const mapped = filtered.map(log => {
          const timestamp = log.timestamp;
          const tel = log.phoneNumber || 'Inconnu';
...
/**
 * Log pour les envois via send.html (eelogs)
 * @param {Object} logData - Données à logger
 * @param {string} logData.phoneNumber - Numéro envoyé
 * @param {string} logData.status - 'success' ou 'error'
 * @param {string} [logData.error] - Message d'erreur si échec
 * @param {string} [logData.message] - Contenu du message envoyé
 * @param {Object} [logData.media] - Infos média si présent
 */
function logEESend(logData) {
  try {
    const timestamp = DateTime.now().setZone('Africa/Casablanca').toISO();
    const entry = {
      timestamp,
      ...
      media: logData.media || null
    };
    const file = path.join(LOG_DIR, 'eelogs.log');
    fs.appendFileSync(file, JSON.stringify(entry) + '\n', 'utf8');
    return entry;
  } catch (e) {
    console.error('[eelogs] Erreur écriture log:', e);
    return null;
  }
}
